...
import net.mcreator.makingthismod.network.CrusherGuiButtonMessage;
...
public class CrusherGuiScreen extends AbstractContainerScreen<CrusherGuiMenu> {
	...
	private static final ResourceLocation texture = new ResourceLocation("making_this_mod:textures/screens/crusher_gui.png");
	...
	public void init() {
		super.init();
		button_crush = Button.builder(Component.translatable("gui.making_this_mod.crusher_gui.button_crush"), e -> {
			if (true) {
				MakingThisModMod.PACKET_HANDLER.sendToServer(new CrusherGuiButtonMessage(0, x, y, z));
				CrusherGuiButtonMessage.handleButtonAction(entity, 0, x, y, z);
			}
		}).bounds(this.leftPos + 62, this.topPos + 41, 51, 20).build();
		guistate.put("button:button_crush", button_crush);
